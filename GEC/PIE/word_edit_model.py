...
class GECProcessor(DataProcessor):
  ...
  def get_dev_examples(self, data_dir):
    """See base class."""
    dev_incorr = self._read_file(os.path.join(data_dir, "X_test.txt"))
    dev_labels = self._read_file(os.path.join(data_dir, "y_test.txt"))
...
def main(_): 
  ...
  if FLAGS.do_predict:
    ...
    num_test_examples = get_file_length(os.path.join(FLAGS.data_dir, "X_test.txt"))
